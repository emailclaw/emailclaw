---
name: guidance
description: "Answer user questions about Emailclaw installation and configuration: first locate and read local documentation, then distill the answer; if local information is insufficient, fall back to the official website documentation."
metadata:
...
# Emailclaw Installation and Configuration Q&A Guide

Use this skill when the user asks about **Emailclaw installation, initialization, environment configuration, dependency requirements, or common configuration options**.
...
DOC_DIR=$(find ~/emailclaw/memory/ -type d -name "docs")
```
...
Run the following script logic to obtain the variable $EMAILCLAW_ROOT:

```bash
# Get the absolute path of the binary
COP_PATH=$(which emailclaw 2>/dev/null || whereis emailclaw | awk '{print $2}')

# Logical deduction: if the path contains emailclaw/bin/emailclaw, the root is three levels up
# Example: /path/to/Emailclaw/emailclaw/bin/emailclaw -> /path/to/Emailclaw
if [[ "$COP_PATH" == *"emailclaw/bin/emailclaw" ]]; then
    EMAILCLAW_ROOT=$(echo "$COP_PATH" | sed 's/\/emailclaw\/bin\/emailclaw//')
else
    # Fallback: try to get the parent of the parent directory
    EMAILCLAW_ROOT=$(dirname $(dirname "$COP_PATH") 2>/dev/null || echo ".")
fi

echo "Detected Emailclaw Root: $EMAILCLAW_ROOT"
```

Verify and list the documentation directory:
Use the derived $EMAILCLAW_ROOT to locate the documentation:

```bash
# Construct the standard documentation path
DOC_DIR="$EMAILCLAW_ROOT/website/public/docs/"
...
if [ -d "$DOC_DIR" ]; then
    ...
    find "$EMAILCLAW_ROOT" -type d -name "docs" | grep "website/public/docs"
fi
```

**If project documentation does not exist, search the working directory**

If documentation is still not found, search for available documentation content under the emailclaw installation path:
