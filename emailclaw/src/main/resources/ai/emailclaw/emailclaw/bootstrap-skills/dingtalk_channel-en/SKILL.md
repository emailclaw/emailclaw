...
description: "Use a headed browser to automatically complete DingTalk channel integration for Emailclaw. Applicable when the user mentions DingTalk, developer console, Client ID, Client Secret, bot, Stream mode, binding or configuring a channel. Supports pausing when a login page is detected and resuming after the user logs in."
metadata:
...
This skill automates the creation of a DingTalk application and the binding of a Emailclaw channel using a headed browser.
...
4. If the user provides no custom values, you must first explicitly reply:
   - "All default settings will be used (Emailclaw / Your personal assistant / default images). Proceeding now."
...
2. Fill in the application information (prefer user-customized values, otherwise use defaults):
   - Application name: default `Emailclaw`
