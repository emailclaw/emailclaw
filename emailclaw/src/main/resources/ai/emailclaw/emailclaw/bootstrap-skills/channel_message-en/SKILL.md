...
description: Use this skill to proactively send a one-way message to a user/session/channel, usually only when the user explicitly asks to send to a channel/session or when proactive notification is needed. First query sessions with emailclaw chats list, then push with emailclaw channels send.
metadata:
...
### Should Not Use
- If you are simply replying in the current session, **do not use `emailclaw channels send`**
...
```bash
emailclaw chats list --agent-id <your_agent> --channel <channel>
```

You can also filter by user:

```bash
emailclaw chats list --agent-id <your_agent> --user-id <user_id>
```

### 2) Send a message

```bash
emailclaw channels send \
  --agent-id <your_agent> \
  --channel <channel> \
  --target-user <user_id> \
...
`emailclaw channels send` requires all of the following:
- `--agent-id`
- `--channel`
- `--target-user`
- `--target-session`
...
`emailclaw channels send` only sends — it does not wait for a reply.
...
```bash
emailclaw chats list --agent-id notify_bot --channel feishu
...
```bash
emailclaw chats list --agent-id task_bot --channel console
...
```bash
emailclaw chats list --agent-id analyst_bot --user-id alice
...
If you are replying to the user in the current session, do not use `emailclaw channels send`.
...
```bash
emailclaw chats list --agent-id <your_agent>
```
...
```bash
emailclaw channels list --agent-id <your_agent>
```
...
- **emailclaw agents chat**: sends to another agent, two-way, returns a reply
- **emailclaw channels send**: sends to a user/session/channel, one-way, no reply

**Selection principle**:
- Need to collaborate with another agent → `emailclaw agents chat`
- Need to proactively push a message to a user → `emailclaw channels send`

---

## Full Parameter Reference

### emailclaw chats list
...
- `--target-user`: target user ID (obtained from `emailclaw chats list`)
- `--target-session`: target session ID (obtained from `emailclaw chats list`)
...
```bash
emailclaw channels -h
emailclaw channels send -h
emailclaw chats -h
emailclaw chats list -h
```
