...
description: Use this skill when another agent's expertise or context is needed, or when the user explicitly asks to involve another agent. First list agents, then use emailclaw agents chat for two-way communication with replies.
metadata:
...
```bash
emailclaw agents list
```

### 2) Start a New Conversation (Real-time Mode)

```bash
emailclaw agents chat \
...
```bash
emailclaw agents chat --background \
  --from-agent <your_agent> \
  --to-agent <target_agent> \
  --text "[Agent <your_agent> requesting] ..."
```
...
```bash
emailclaw agents chat --background --task-id <task_id>
```
...
|-----------|-------------|---------|
| Simple quick query | Real-time mode | `emailclaw agents chat` |
| Complex task (data analysis, batch processing, etc.) | Background mode | `emailclaw agents chat --background` |
...
2. emailclaw agents list
3. emailclaw agents chat to start a conversation
...
2. emailclaw agents list
3. emailclaw agents chat --background to submit the task
...
`emailclaw agents chat` must include all of the following:
- `--from-agent`
- `--to-agent`
- `--text`
...
Check status with:
  emailclaw agents chat --background --task-id 20802ea3-...
```
...
sleep 30 && emailclaw agents chat --background \
  --task-id 20802ea3-832d-4fb4-86f0-666ad79fcc80
```
...
emailclaw agents chat --background --task-id <id>
```

**Method 2: Timed polling** (if you must wait)
```bash
# Incrementally increasing intervals, fast first then slow
sleep 10 && emailclaw agents chat --background --task-id <id>
sleep 20 && emailclaw agents chat --background --task-id <id>
sleep 30 && emailclaw agents chat --background --task-id <id>
```
...
```bash
emailclaw agents -h
emailclaw agents list -h
emailclaw agents chat -h
```
