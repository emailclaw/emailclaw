---
name: guidance
description: "回答用户关于 Emailclaw 安装与配置的问题：优先定位并阅读本地文档，再提炼答案；若本地信息不足，兜底访问官网文档。"
metadata:
...
当用户询问 **Emailclaw 的安装、初始化、环境配置、依赖要求、常见配置项** 时，使用本 skill。
...
执行以下脚本逻辑来获取变量 $EMAILCLAW_ROOT：

```bash
# 获取二进制绝对路径
COP_PATH=$(which emailclaw 2>/dev/null || whereis emailclaw | awk '{print $2}')

# 逻辑推导：如果路径包含 emailclaw/bin/emailclaw，则根目录在其上三层
# 例如：/path/to/Emailclaw/emailclaw/bin/emailclaw -> /path/to/Emailclaw
...
如果还是找不到文档，搜索 emailclaw 安装路径下的可用文档内容：
