...
- This **attaches to an external browser**, not a new process launched by Emailclaw
...
### 1. Managed CDP launched by Emailclaw
...
These browsers are launched and owned by Emailclaw. `stop` will:
...
These browsers are not launched by Emailclaw. `stop` will only:
